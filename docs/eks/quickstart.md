...
The QuickStart steps for installing the Currents Helm Chart on EKS quickly runs through the commands to get the Currents App setup. This includes installing MongoDB, Elasticsearch, and Minio into your Kubernetes namespace well. See the full Chart documentation for alternative configuration options for these services. 

## Cluster prerequisites

- Have an supported EKS cluster running
  * SSD based StorageClass available for use
  * An ALB IngressClass setup that can be used by Currents
- The ability to register two new subdomains for Currents (for the server, and the director)
  * The `sever` is often called the `app` and includes the web frontend and REST API endpoints.
  * The `director` is sometimes called the `api`, and the endpoint the test reporters communicate with.
...
- Your EKS Nodes need to have been given IAM permissions to pull images from Current's Private ECR repositories
...
## Setup prerequisites services

These are the services that Currents depends on, and are provided by their developers.
